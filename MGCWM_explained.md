...
$$
p^R(x,y) = \sum_{m=1}^{M}\left\{\sum_{k\in G_m^{}}\eta_{mk}\,\phi_p(x;\mu_k,\Sigma_k)\,\phi_1\big(y;(1,x^\top)\beta_m,\sigma_k^2\big)\right\}
$$
...
가장 작은 $d_{jk}$를 갖는 쌍 $(j^*,k^*)$부터 순서대로 시도한다.
...
$$
G_1^R,\dots,G_{M-1}^R \;=\; \big(G_1,\dots,G_M\big)\ \text{에서}\ G_{j^*},G_{k^*}\ \text{를 하나로 합친 것}
$$
...
$$
\underbrace{\text{GCWM}(K)}_{\texttt{gcwm.py}}
\;\xrightarrow{\ \texttt{groups}=[[k]]\ }\;
\underbrace{\_refit\_partition}_{\text{E-step}\leftrightarrow\text{M-step, Eq.9}}
\;\xrightarrow{\ d_{jk}\text{ 최솟값 탐색}\ }\;
\underbrace{\text{후보 병합}}_{\text{trial\_groups}}
\;\xrightarrow{\ \text{BIC/ICL 비교}\ }\;
\underbrace{\text{확정 or 기각}}_{\text{Algorithm 1 (4)}}
$$
