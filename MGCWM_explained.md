...
즉 $X$의 분포(위치·모양)는 원래 컴포넌트별 값을 그대로 갖지만, $Y\mid X$의 회귀선(평균)은 같은 클러스터에 속한 컴포넌트끼리 **공유**한다. 하지만 분산은 달라야한다. 그 이유는 분산마저 같아버리면 하나의 데이터가 어떤 분포에서 나왔는지 알 수 없는 비식별성 문제가 발생하기 때문이다.
