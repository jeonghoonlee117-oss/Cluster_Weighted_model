...
이 관계는 코드에서 컴포넌트 → 클러스터 매핑 배열 하나로 구현된다. 단, 하나의 컴포넌트는 하나의 군집에 속한다는 전제가 가정되어 있다. 더욱 직관적으로 이해하면 다음과 같다. 1반부터 5반까지 있다고 가정하였을 때, 1~3반은 A코스의 학습을 수행하고 4~5반은 B코스의 학습을 수행한다. 1반이라는 정보만 있으면 A코스를 학습한다는 사실을 명확하게 알 수 있지만 A코스를 수강하고 있다고 해서 이 반이 어떤 반인지에 대한 확실한 정보를 얻을 수 없다.
...
즉 $X$의 분포(위치·모양)는 원래 컴포넌트별 값을 그대로 갖지만, $Y\mid X$의 회귀선(평균)은 같은 클러스터에 속한 컴포넌트끼리 공유한다. 왜냐하면 beta의 기준으로 유사한 것들로 묶기 때문이다. 하지만 분산은 달라야 한다. 그 이유는 분산마저 같아버리면 이 컴포넌트가 어떤 군집에서 나왔는지 명확하게 알 수 없기 때문이다.
...
> $\beta_m^{(0)}$는 GCWM에서 얻은 β들의 단순 산술평균일 뿐이며, 아직 이 파티션(β 공유 제약) 하에서 최적화된 값이 아니다. 최종적으로 쓰이는 $\hat\beta_m$은 아래 3.2~3.3의 E-step·M-step을 여러 번 반복해서 수렴한 값이며, 초깃값과 다른 가중 평균으로 업데이트 된다는 점이다.
...
즉 β_m은 "평균 한 번 내고 끝"이 아니라, **"평균으로 시작 → (사후확률 계산 ↔ 가중 WLS로 재추정)을 반복 → 수렴"**이라는 순서를 거친다. E-step의 $w_{imk}^{(t)}$와 M-step의 $\beta_m^{(t+1)}$은 서로를 갱신해주며 사후확률에 영향을 주고, 그 값으로 모수를 갱신하는 과정인 것이다.
